...
gulp.task("move-image-low", () => {
  return gulp.src(srcImageRecursivePath).pipe(gulp.dest(distLqImagePath));
});
gulp.task("move-image-high", () => {
  return gulp.src(srcImageRecursivePath).pipe(gulp.dest(distHqImagePath));
});
gulp.task("optimize-image-low", () => {
  return gulp
    .src(srcImageRecursivePath)
    .pipe(
      imagemin([
...
gulp.task("move-css", () => {
  return gulp
    .src(distCssPath + "/style_merged.css")
...
gulp.task(
  ...
  gulp.series(
    ...
    () => {
      ...
      gulp.watch(
        "src/assets/js/**/*.js",
        gulp.series(["build-js", "bundle-js", "move-js", reload])
      );
      ...
      gulp.watch(
        ...
        gulp.series(["build-sass", "bundle-css", "move-css", reload])
      );

      gulp.watch(
        "src/views/**/*.html",
        gulp.series(["build-html", "move-css", reload])
      );
    }
  )
);
